...
of Roger Light <[roger@atchoo.org](mailto:roger@atchoo.org)> from the [paho-project](https://eclipse.org/paho/).
...
### mqtt.\__init__(client_id, clean_session=True)
...
!!!abstract "`#!py3 on(command, function, condition=None, priority=0)`"
...
!!!abstract "`#!py3 set_will(topic, payload, qos, retain)`"
...
!!!abstract "`#!py3 set_username_pw(username, password = None)`"
...
!!!abstract "`#!py3 connect(host, keepalive, port=1883, ssl_ctx=None, breconnect_cb=None, aconnect_cb=None, sock_keepalive=None)`"
...
!!!abstract "`#!py3 unsubscribe(topics)`"
...
!!!abstract "`#!py3 publish(topic, payload=None, qos=0, retain=False)`"
...
!!!abstract "`#!py3 loop(on_message = None)`"
