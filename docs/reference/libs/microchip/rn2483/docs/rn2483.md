...
* *ser*```ser``` serial port used for device-to-module communication (ex. SERIAL1)
* *rst*
...
As a raw value passing *raw*```raw``` as True.
...
* *ser*```ser``` is the serial port used for device-to-module communication (ex. SERIAL1)
* *appeui*, *
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbODQ4Njk4MTA4XX0=
-->