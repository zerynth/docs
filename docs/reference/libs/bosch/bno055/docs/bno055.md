...
!!!abstract "`#!py3 BNO055(i2cdrv, addr=0x28, clk=100000)`**"
...
** ```Arguments:**```
 ...
 - **    * ```mode`**`` – Mode value selectable from Mode table (refer to page 20 of the BNO055 datasheet), default None
...
** ```Arguments:**```
 ...
 - **    * ```mode**``` – Mode value (refer to page 20 of the BNO055 datasheet)
...
!!!abstract "`#!py3 get_calibration_status()`**"
...
!!!abstract "`#!py3 get_calibration(raw=False)`**"
...
!!!abstract "`#!py3 set_calibration(data, raw=False)`**"
...
** ```Arguments:**
 ...
 - **```
    ...
    * ```data**``` – List of values (11 elements) representing the sensors offsets and radius.
...
Returns temp.
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTYxOTg5ODA3OSwtOTE4Njg3NjQyLDI4ND
kwOTEyMl19
-->